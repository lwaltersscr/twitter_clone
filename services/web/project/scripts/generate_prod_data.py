...
def generate_batch(args):
    ...
    words = ['Hello', 'World', 'Testing', 'Twitter', 'Clone', 'Project', 
             ...
             'Microservices', 'Scalability', 'Testing', 'Deployment',
             'Infrastructure', 'Security', 'Optimization', 'Analytics']
    ...
    session.commit()
    print(f"Created users {start_user} to {end_user}")
    ...
    for user in users:
        for _ in range(tweets_per_user):
            ...
            if len(tweets_batch) >= batch_size:
                session.add_all(tweets_batch)
                session.commit()
                tweets_batch = []
                print(f"Processed {batch_size} tweets for user range {start_user}-{end_user}")
...
def generate_production_data(db_url, num_users=50000, tweets_per_user=200):
    ...
    print(f"Target: {num_users} users with {tweets_per_user} tweets each")
    print(f"Total tweets to generate: {num_users * tweets_per_user}")
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        ...
        print("\nExample for generating 10M+ tweets:")
        print("python generate_prod_data.py postgresql://user:pass@localhost:5432/db 50000 200")
        sys.exit(1)
